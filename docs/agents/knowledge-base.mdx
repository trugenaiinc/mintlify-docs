...
We support two modes or interaction with Knowledge Base's while the agent responds,
  1. `agentic` mode (**Default**); here the LLM takes a decision to search each knowledge base depending on the user query.
  2. Traditional `rag` mode; where each user query is searched against the knowledge base.
...
**Name** and **Description** of the knowledge base is the one of the most important component; these are the fields the agent user's to decide to use the knowledge base or not.
